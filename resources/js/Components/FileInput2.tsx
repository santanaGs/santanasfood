...
export default ({ className, name, label, accept, errors = [], onChange }) => {
	...
	return (
		<div className={className}>
			{label && (
				<label
					className="block text-gray-700 select-none text-sm font-medium"
					htmlFor={name}
				>
					{label}:
				</label>
			...
			<div
				className={`p-2 h-42px block w-full border border-gray-200 text-gray-700 bg-white font-sans rounded text-left appearance-none relative focus:ring-indigo-400/50 focus:ring ${errors.length && "error"
					}`}
			>
				<input
					id={name}
